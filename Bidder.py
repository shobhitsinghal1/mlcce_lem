...
class ProsumerStorage(Bidder):
    def __init__(self, name: str, horizon: int):
        ...
        self.gpenv = gp.Env(params={'LogToConsole': 0, 'OutputFlag': 0})
        self.dq_model = self.__construct_dq_model()
        self.value_model = self.__construct_value_model() # just an additional constraint for bundle, and objective with zero prices
        self.capacity_model = self.__construct_dq_model()
        self.full_info_imp = False


    def __get_param_values(self, param_name: str) -> dict:
        param_values = {}
    ...
    def __add_asset_constraints(self, m: dict):
        ...
        m['model'].addConstrs((m['ps'][h] <= m['M'] * m['delta'][h] for h in m['H']), name=f'cons1_{self.name}')
        m['model'].addConstrs((m['ps'][h] >= -m['M'] * (1 - m['delta'][h]) for h in m['H']), name=f'cons2_{self.name}')
        m['model'].addConstrs((m['s'][h-1] + m['eta'] * m['ps'][h] - m['gamma'][h] - m['M'] * (1 - m['delta'][h]) <= m['s'][h] for h in m['H']), name=f'cons3l_{self.name}')
        m['model'].addConstrs((m['s'][h] <= m['s'][h-1] + m['eta'] * m['ps'][h] - m['gamma'][h] + m['M'] * (1 - m['delta'][h]) for h in m['H']), name=f'cons3u_{self.name}')
        m['model'].addConstrs((m['s'][h-1] + m['ps'][h] / m['eta'] - m['gamma'][h] - m['M'] * m['delta'][h] <= m['s'][h] for h in m['H']), name=f'cons4l_{self.name}')
        m['model'].addConstrs((m['s'][h] <= m['s'][h-1] + m['ps'][h] / m['eta'] - m['gamma'][h] + m['M'] * m['delta'][h] for h in m['H']), name=f'cons4u_{self.name}')
        ...
        m['model'].addConstr((m['s'][-1] == m['s0']), name=f'cons11_{self.name}')

        m['model'].addConstrs((m['P'][h] >= m['ps'][h] for h in m['H']), name=f'cons9_{self.name}')
        m['model'].addConstrs((m['P'][h] >= -m['ps'][h] for h in m['H']), name=f'cons10_{self.name}')
        m['model'].addConstrs((m['P'][h] <= m['ramp_limit'] for h in m['H']), name=f'cons7_{self.name}')
    ...
    def __get_value_obj_expr(self, m: dict):
        return gp.quicksum( - m['P'][h]*m['beta'][h] + m['oppcprice'][h] * m['ps'][h] * m['delta'][h] + m['oppdprice'][h] * m['ps'][h] * (1 - m['delta'][h]) for h in m['H'])
    ...
    def __add_dq_objective(self, m: dict, price: np.ndarray):
        ...
        m['model'].setObjective(
            self.__get_value_obj_expr(m)
            - gp.quicksum(m['ps'][h]*price[h] for h in m['H']),
            GRB.MAXIMIZE
        )
    ...
    def __add_asset_params_vars(self, m: dict):
        ...
        # preference parameters
        m['oppcprice'] = self.__get_param_values('oppcprice') # (h) lower limit violation penalty coeff
        m['oppdprice'] = self.__get_param_values('oppdprice') # (h) upper limit violation penalty coeff
        m['available'] = self.__get_param_values('available')
        m['su'] = self.__get_param_values('su') # (h) SoC upper limit
        m['sl'] = self.__get_param_values('sl') # (h) SoC lower limit
        m['s0'] = self.__get_param_values('s0') # () initial SoC
        m['eta'] = self.__get_param_values('eta') # () charging efficiency
        m['gamma'] = self.__get_param_values('gamma') # (h) dissipation rate
        m['beta'] = self.__get_param_values('beta') # (h) storage degradation coeff
        m['ramp_limit'] = self.__get_param_values('ramp_limit') # () ramp limit

        # variables
        m['ps'] = m['model'].addVars(m['H'], name=f'ps_{self.name}', lb=float('-inf')) # power for storage
        #   auxiliary: storage
    ...
    def __construct_value_model(self, ) -> dict:
        ...
        self.__add_asset_params_vars(model)
        self.__add_asset_constraints(model)
        model['con_bundle'] = model['model'].addConstrs((model['ps'][h] == 0 for h in model['H']), name=f'con_bundle_{self.name}')
        self.__add_dq_objective(model, np.zeros(self.horizon))
    ...
    def add_model(self, model: gp.Model) -> tuple:
        ...
        return model['ps'], self.__get_value_obj_expr(model)
    ...
    def bundle_query(self, price: np.ndarray) -> tuple:
        ...
        bundle = np.array([self.dq_model['ps'][h].x for h in self.dq_model['H']])
        return bundle, self.dq_model['model'].ObjVal + np.dot(price, bundle)
    ...
    def get_capacity_generic_goods(self, ) -> np.ndarray:
        ...
        for i in range(self.horizon):
            bounds.append([])
            for sense in [-1, 1]:
                self.capacity_model['model'].setObjective(self.capacity_model['ps'][i] * sense, GRB.MAXIMIZE)
                self.capacity_model['model'].update()
                self.capacity_model['model'].optimize()
                bounds[i].append(np.abs(self.capacity_model['ps'][i].x))
        return np.max(bounds, axis=1)
